// Auto-generated feature route: competitor-scrape
// Domain: competitor copy analyst — Capability: scraping competitors and suggesting counter-positioning
// Mounted under: /api/competitor-scrape
...
// POST /api/competitor-scrape/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a competitor copy analyst. Your task is scraping competitors and suggesting counter-positioning. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/competitor-scrape/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/competitor-scrape/:id — fetch one
...
// DELETE /api/competitor-scrape/:id — discard a run
