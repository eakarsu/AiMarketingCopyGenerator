...
// Route: /api/gap-cross-channel
// Feature: Cross Channel
// Description: Cross-channel performance analytics
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Cross Channel. Cross-channel performance analytics' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'cross-channel', title: 'Cross Channel', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'cross-channel', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['cross-channel']);
      return res.json({ slug: 'cross-channel', items: r.rows || [] });
    }
    return res.json({ slug: 'cross-channel', items: [] });
  } catch (e) {
    return res.json({ slug: 'cross-channel', items: [], error: (e && e.message) || 'Server error' });
  }
});
