...
// Route: /api/gap-scheduling
// Feature: Scheduling
// Description: Scheduling & publishing automation
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Scheduling. Scheduling & publishing automation' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'scheduling', title: 'Scheduling', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'scheduling', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['scheduling']);
      return res.json({ slug: 'scheduling', items: r.rows || [] });
    }
    return res.json({ slug: 'scheduling', items: [] });
  } catch (e) {
    return res.json({ slug: 'scheduling', items: [], error: (e && e.message) || 'Server error' });
  }
});
