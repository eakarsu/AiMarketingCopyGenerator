// Auto-generated feature route: brand-voice-enforcer
// Domain: brand voice drift detector — Capability: detecting brand voice drift across channels
// Mounted under: /api/brand-voice-enforcer
...
// POST /api/brand-voice-enforcer/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a brand voice drift detector. Your task is detecting brand voice drift across channels. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/brand-voice-enforcer/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/brand-voice-enforcer/:id — fetch one
...
// DELETE /api/brand-voice-enforcer/:id — discard a run
