// Auto-generated feature route: vertical-compliance-templates
// Domain: compliance-aware copy librarian — Capability: producing medical/legal/SaaS/ecommerce templates
// Mounted under: /api/vertical-compliance-templates
...
// POST /api/vertical-compliance-templates/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a compliance-aware copy librarian. Your task is producing medical/legal/SaaS/ecommerce templates. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
    ...
    const record = {
      ...
      slug: 'vertical-compliance-templates',
      input: payload,
      output: parsed,
...
// GET /api/vertical-compliance-templates/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/vertical-compliance-templates/:id — fetch one
...
// DELETE /api/vertical-compliance-templates/:id — discard a run
