// Auto-generated feature route: copy-optimizer-agent
// Domain: agentic copy optimizer — Capability: batch-testing variants on ad platforms and evolving winners
// Mounted under: /api/copy-optimizer-agent
...
// POST /api/copy-optimizer-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a agentic copy optimizer. Your task is batch-testing variants on ad platforms and evolving winners. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/copy-optimizer-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/copy-optimizer-agent/:id — fetch one
...
// DELETE /api/copy-optimizer-agent/:id — discard a run
