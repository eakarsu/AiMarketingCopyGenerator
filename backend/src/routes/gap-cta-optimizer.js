...
// Route: /api/gap-cta-optimizer
// Feature: Cta Optimizer
// Description: (call-to-action variant generator)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Cta Optimizer. (call-to-action variant generator)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'cta-optimizer', title: 'Cta Optimizer', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'cta-optimizer', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['cta-optimizer']);
      return res.json({ slug: 'cta-optimizer', items: r.rows || [] });
    }
    return res.json({ slug: 'cta-optimizer', items: [] });
  } catch (e) {
    return res.json({ slug: 'cta-optimizer', items: [], error: (e && e.message) || 'Server error' });
  }
});
